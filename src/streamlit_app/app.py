...
def simple_markdown_format(text: str) -> str:
    ...
    sections = [
        'Missions principales', 'De formation technique',
        'Les avantages', 'Vous maîtrisez', 'Rigoureux',
        'Profil recherché', 'Vos missions', 'Compétences requises'
    ]
...
def create_timeline_chart(df_offers: pd.DataFrame) -> go.Figure | None:
    ...
    try:
        ...
        df_timeline['date_creation'] = pd.to_datetime(
            df_timeline['date_creation'],
            errors='coerce'
        )
        ...
        with col4:
            if 'date_actualisation' in df_offers.columns:
                latest = pd.to_datetime(
                    df_offers['date_actualisation'],
                    errors='coerce'
                ).max()
                if pd.notna(latest):
                    display_stat_card("Dernière MAJ", latest.strftime('%d/%m'), latest.strftime('%Y'))
...
with tab3:
    if not df_offers.empty:
        ...
        with col1:
            ...
            default_cols = [
                'intitule', 'entreprise_nom', 'lieu_travail',
                'type_contrat_libelle', 'date_creation'
            ]
